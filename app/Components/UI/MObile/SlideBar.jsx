...
function SlideBar() {
  ...
  const renderSubCategories = (subs, categoryId, level = 0) => {
    ...
    return subs.map((sub, idx) => {
      ...
      const isExpanded = activeSubCategory[subKey]
            ...
            <button
              onClick={() => {
                if (hasNestedSubs) {
                  ...
                  setSelectedSubCategory(typeof sub === 'string' ? sub : sub.name)
                  performAction()
                }
              }}
                ...
                <div className="space-y-3">
                  {categories.map((cat, index) => (
                    ...
                    >
                      ...
                      <div
                        onClick={() => {
                          setActive(active === cat._id ? 0 : cat._id)
                      ...
                      <div className={`h-[2px] bg-gradient-to-r from-slate-300 to-slate-500 transition-all duration-500 ${
                        active === cat._id ? 'scale-x-100' : 'scale-x-0'
                      } origin-left`} />

                     
                      <div className={`transition-all duration-500 ease-out overflow-hidden ${
                        active === cat._id ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
                      }`}>
                        {cat.subs?.length > 0 ? (
          ...
          <div className="h-full flex flex-col">
            ...
            <div className="p-6">
              <div className="relative group">
                ...
                <div className="relative flex items-center bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all duration-300">
                  ...
                  <button
                    ...
                    className="ml-3 p-2 bg-slate-800 hover:bg-slate-900 text-white rounded-xl transition-all duration-300 hover:scale-105 disabled:opacity-50"
              ...
              <div className="mt-6 space-y-2">
                ...
                {['Electronics', 'Fashion', 'Home & Garden', 'Sports'].map((item, idx) => (
                  <button
                    key={idx}
